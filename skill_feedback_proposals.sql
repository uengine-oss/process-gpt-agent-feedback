...
CREATE TABLE IF NOT EXISTS public.skill_feedback_proposals (
...
CREATE UNIQUE INDEX IF NOT EXISTS skill_feedback_proposals_collecting_key
    ON public.skill_feedback_proposals (tenant_id, proc_def_id, activity_id)
    WHERE status = 'COLLECTING';

CREATE INDEX IF NOT EXISTS skill_feedback_proposals_status_idx
    ON public.skill_feedback_proposals (status);

CREATE TRIGGER skill_feedback_proposals_updated_at
    BEFORE UPDATE ON public.skill_feedback_proposals
    FOR EACH ROW
...
CREATE OR REPLACE FUNCTION public.append_feedback_to_batch(
    ...
    p_time        text,
    p_user_id     text
) RETURNS public.skill_feedback_proposals
LANGUAGE plpgsql
AS $$
DECLARE
    v_item  jsonb;
    v_batch public.skill_feedback_proposals;
BEGIN
    ...
    INSERT INTO public.skill_feedback_proposals (
    ...
    DO UPDATE SET
        collected_items = public.skill_feedback_proposals.collected_items || jsonb_build_array(v_item)
...
ALTER TABLE public.skill_feedback_proposals
